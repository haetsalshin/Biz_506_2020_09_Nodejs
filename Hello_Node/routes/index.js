...
/* GET home page. */
// 쌤) localhost:3000/ 를 GET반식으로 요청을 하면
// function() 함수를 실행하라
// router.get() 콜백함수, 함수
// function(req,res) {}
// req(request) : client(web browser)가 요청하는 여러가지 정보를 담아
//    서버에 전달해주는 역할 수행
// res(response) : 서버가 client에게 응답하는 것들이 담겨있다.
router.get('/', function(req, res, next) {
  // views/index.pug파일을읽어서 rendering 한 후
  // client에게 응답하라
...
// 쌤) localhost:3000/home으로 요청을 하면
...
// 숫자1에 있는 값을 추출, 숫자 2에 있는 값을 추출하여
// 정수로 변환한다음 덧셈하여 결과를 response하라
router.get("/add/:num1/:num2",function(req,res){
  // req를 통해 params라는 객체를 통해 num1를 숫자로 바꿔서 사용해라
